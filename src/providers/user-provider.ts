...
export class UserProvider {
  baseUrl: string = 'http://mydesignsandbox.com/demo/archi1/wp-json';

  constructor(public events: Events, private httpInterceptor: HttpClient, private stor: Storage) {
  }
  ...
  login(username, password) {
    ...
    this.httpInterceptor.post(`${this.baseUrl}/jwt-auth/v1/token`, data).map(res => res.json())
      .subscribe(response => {
        ...
        this.httpInterceptor.addHeader('Authorization', `Bearer ${response.token}`);
  ...
  get() {
    this.httpInterceptor.get('http://mydesignsandbox.com/demo/archi1/wp-json/wp/v2/posts/?_embed')
      .subscribe( res => {
        console.log(res);
      })
  }
